...
A URDF gives you a robot's geometry and its kinematic tree. Ingesting it means
two API calls on one `rerun.urdf.UrdfTree`: stream the static model, then drive
it with forward kinematics from joint states. The transforms it produces are
derived, so they are a **layer**, never base (the `URDF + joints (computed)` row
of the `rerun-data-model` table).
...
The tree is also introspectable, and several judgment calls below depend on
that: `urdf.joints()` returns every `UrdfJoint` with `name`, `joint_type`,
`parent_link`, `child_link`, `axis`, `origin_xyz`/`origin_rpy`, limits, and
`mimic` (a `UrdfMimic` with driver `joint`, `multiplier`, `offset`, or `None`).
`get_joint_by_name`, `get_link_by_name`, `root_link()`, and
`get_visual_geometry_paths(link)` round it out. For one-off, non-stream use
there are `joint.compute_transform(value)`, `joint.compute_transform_columns(values)`
(feeds `rr.send_columns`), and `urdf.log_urdf_to_recording()` to log the whole
model through the classic logging API (the `animated_urdf` example,
...
Recolor a robot's meshes with a `MutateLens` on `Asset3D:albedo_factor` (see
`rerun-chunk-processing`).

**2. Solve forward kinematics.** Given joint names and the matching joint values,
it returns one `rerun.urdf.JointTransformBatch` per input row. Each batch is a
list of per-joint entries with `parent_frame`, `child_frame`, `translation`, and
`quaternion`:
...
source produces an empty FK layer **with no error**. The most common cause is
a protobuf MCAP whose schema ships without a compiled descriptor; the fix
(patching the descriptor into the MCAP before any reader touches it) is in
`rerun-mcap`. Whatever the source, confirm the joint-state stream yields rows
before debugging FK.
...
fk = (
    ...
    .lenses(
        ...
        content=JOINT_TOPIC, output_mode="forward_all",
    )
    .lenses(
...
`read_names`, `read_values`, `JOINT_MSG_COMPONENT`, and `JOINT_TOPIC` are the
only data-specific pieces, and the mapping section above is what makes them
correct.

## Gotchas that cause real failures

1. Empty layer, no error: dead joint-state source (e.g. unpatched protobuf
   MCAP descriptor, see `rerun-mcap`) or wrong `JOINT_TOPIC`/component name.
...
5. Scene looks merged at the origin: unconnected roots logged as identity
   without a calibration edge.
6. Catalog ingest rejects or mis-orders chunks: `OBJECT_STORE` optimization
   skipped.

## Verify before relying on these

Verified against `rerun-sdk 0.34.0a1`. `UrdfTree`, `JointTransformBatch`, and
the `Transform3D.descriptor_*` accessors are `rerun.urdf` /
`rerun.experimental` surface; on upgrade re-check with
`python -c "from rerun.urdf import UrdfTree; help(UrdfTree)"`.
...
- `rerun-mcap` (making MCAP joint-state topics readable)
