...
catalog from Python (`rerun.catalog.CatalogClient` →
...
src_a = shared.filter(col(f"{ent_a}:{comp_a}").is_not_null()).select(...)
src_b = shared.filter(col(f"{ent_b}:{comp_b}").is_not_null()).select(...)
```
...
## Use `segment_table()` as a per-segment cache fingerprint

Per-segment metadata is exposed cheaply:

```python
tbl = dataset.segment_table().select(
    "rerun_segment_id",
    "rerun_last_updated_at",  # timestamp[ns]
    "rerun_num_chunks",
    "rerun_size_bytes",
)
```

`rerun_last_updated_at` flips when chunks rewrite. `num_chunks` and
`size_bytes` add belt-and-suspenders against subtle layer-only edits.
Use the triple as a fingerprint key for any per-segment cache (split
windows, segment-context dataclasses, etc.). One query for the whole
dataset; instant cache invalidation downstream.
